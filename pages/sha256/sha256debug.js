...
function sha256(string){
	...
	//Padding the message
	var zeroBits = addFrontZero("",mod(448-(str.length+1), 512));
...
console.log(M);
	...
	for(var i=0; i<M.length; i++){
	  ...
	  for(var t=0; t<64; t++){
		if(t < 16){
		  W.push(parseInt(M[i][t], 2) % Math.pow(2,32));
		}else{
		  temp = (omega1(W[t-2]) + W[t-7] + omega0(W[t-15]) + W[t-16]) % Math.pow(2,32);
		  W.push(temp);
		}
	  }
	  console.log(W);
